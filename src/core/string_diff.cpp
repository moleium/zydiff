...
namespace {
  ...
  [[nodiscard]] auto is_string_section(const binary_parser::section& section) -> bool {
    ...
    if (section.name.contains("debug") || section.name == ".symtab" || section.name == ".strtab" ||
  ...
  [[nodiscard]] auto extract_strings(const binary_parser& parser, size_t min_length)
    -> std::vector<string_diff::string_entry> {
    std::vector<string_diff::string_entry> strings;
  ...
  [[nodiscard]] auto resolve_instruction_address(const binary_parser::section& text, size_t match_offset)
  ...
  void attach_xrefs(
    const binary_parser& parser, std::vector<string_diff::string_entry>& strings, size_t max_xrefs_per_string
  ) {
    ...
    std::unordered_map<size_t, uint64_t> instruction_address_cache;

    auto get_instruction_address = [&](size_t match_offset) {
      auto it = instruction_address_cache.find(match_offset);
      if (it != instruction_address_cache.end()) {
        return it->second;
      }

      const auto resolved = resolve_instruction_address(*text, match_offset).value_or(text->virtual_address + match_offset);
      instruction_address_cache.emplace(match_offset, resolved);
      return resolved;
    };
    ...
    auto add_xref = [&](uint64_t target, size_t match_offset) {
      ...
      const auto xref_address = get_instruction_address(match_offset);
      for (auto string_index : it->second) {
        auto& xrefs = strings[string_index].xrefs;
        if (xrefs.size() < max_xrefs_per_string && !std::ranges::contains(xrefs, xref_address)) {
  ...
  [[nodiscard]] auto diff_strings(
    const std::vector<string_diff::string_entry>& source,
    const std::vector<string_diff::string_entry>& other
  ) -> std::vector<string_diff::string_entry> {
    ...
    std::vector<string_diff::string_entry> result;
...
string_diff::result string_diff::compare(
  const std::string& primary_path, const std::string& secondary_path, options opts
) {
  ...
  attach_xrefs(primary, primary_strings, opts.max_xrefs_per_string);
  attach_xrefs(secondary, secondary_strings, opts.max_xrefs_per_string);

  return {
    .primary_string_count = primary_strings.size(),
    .secondary_string_count = secondary_strings.size(),
